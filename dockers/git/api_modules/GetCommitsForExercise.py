...
def process(api_req, maria_conn, lite_conn):
    ...
    if maria_conn:
        try:
            ...
            if api_req.get('recursive', True):
                sql_request += 'AND (co.exercise_path LIKE %s OR cf.exercise_path LIKE %s) '    # Recursif
                sql_params.extend([ex_path+'%', ex_path+'%'])
            else:
                sql_request += 'AND (co.exercise_path = %s OR cf.exercise_path = %s) '          # Exact match
                sql_params.extend([ex_path, ex_path])
